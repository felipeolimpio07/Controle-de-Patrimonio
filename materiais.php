...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validações
    if ($nome === '' || $origem === '') {
        $msg = "⚠️ Por favor, preencha todos os campos obrigatórios.";
        $msg_type = "error";
    } elseif (strlen($nome) < 3) {
        $msg = "⚠️ O nome do material deve ter no mínimo 3 caracteres.";
        $msg_type = "error";
    } else {
        ...
        if ($check_stmt) {
            ...
            if ($check_stmt->num_rows > 0) {
                $msg = "⚠️ Este material já está cadastrado no sistema.";
                $msg_type = "error";
            } else {
    ...
    <div class="form-container">
        ...
        <form method="POST" action="">
            <div class="input-group">
                <label for="nome">
                    <span class="label-icon">🏷️</span> Nome do Material:
                </label>
                ...
                <input 
                    ...
                    id="nome" 
                    name="nome" 
                    placeholder="Ex: Capacete, Luva, Marreta, etc."
                    value="<?php echo isset($nome) ? htmlspecialchars($nome) : ''; ?>"
                    required 
                    minlength="3"
                />
                <small class="input-hint">Digite o nome do material (mínimo 3 caracteres)</small>
            </div>
